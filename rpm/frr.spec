...
%build
# Grout itself needs GCC Toolset 15 on EL9, and the development container puts
# that compiler first in PATH. FRR 10.6.1 trips a GCC 15/annobin LTO ICE, while
# OBS normally builds it with the EL9 system compiler. Pin FRR to that compiler
# so local source-package validation and OBS use the same supported toolchain.
%if 0%{?rhel} == 9
