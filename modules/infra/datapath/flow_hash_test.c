...
static int setup(void **) {
	gr_mbuf_flow_hash_offset = offsetof(struct rte_mbuf, dynfield1);
	gr_mbuf_flow_hash_flag = RTE_MBUF_F_FIRST_FREE;
	return 0;
}
...
static void canonical_hash_caches_hardware_rss(void **) {
	struct test_packet p;
	...
	assert_true(p.mbuf.ol_flags & RTE_MBUF_F_RX_RSS_HASH);

	p.mbuf.hash.rss = 0x87654321;
	assert_int_equal(gr_mbuf_flow_hash_get(&p.mbuf), 0x12345678);
}

static void canonical_software_hash_leaves_rss_untouched(void **) {
	struct test_packet p;
	uint32_t expected;
	...
	assert_true(gr_mbuf_flow_hash_is_valid(&p.mbuf));
	assert_int_equal(p.mbuf.hash.rss, 0x12345678);
	assert_false(p.mbuf.ol_flags & RTE_MBUF_F_RX_RSS_HASH);
}

static void canonical_software_hash_distinguishes_udp_flows(void **) {
	struct test_packet a, b;
...
static void canonical_l3_ipv4_hash_matches_ethernet_hash(void **) {
	struct test_packet p;
	uint32_t expected;
...
static void canonical_l3_ipv6_hash_matches_ethernet_hash(void **) {
...
static void canonical_l3_hash_imports_hardware_rss(void **) {
	struct test_packet p;
...
static void canonical_ipv4_fragments_share_one_hash(void **) {
	struct test_packet first, later;
...
static void canonical_validity_is_cleared_on_reset(void **) {
	struct test_packet p;
...
static void decap_reseed_quarantines_outer_rss(void **) {
	struct test_packet p;
	uint32_t expected;
	...
	gr_mbuf_flow_hash_decap_reseed(&p.mbuf);
	assert_true(gr_mbuf_flow_hash_is_valid(&p.mbuf));
	assert_int_equal(gr_mbuf_flow_hash_get(&p.mbuf), expected);
	assert_int_equal(p.mbuf.hash.rss, 0x12345678);
	assert_true(p.mbuf.ol_flags & RTE_MBUF_F_RX_RSS_HASH);
...
static void decap_without_rss_invalidates_outer_canonical_hash(void **) {
	struct test_packet p;
...
int main(void) {
	const struct CMUnitTest tests[] = {
		...
		cmocka_unit_test(canonical_hash_caches_hardware_rss),
		cmocka_unit_test(canonical_software_hash_leaves_rss_untouched),
		cmocka_unit_test(canonical_software_hash_distinguishes_udp_flows),
		cmocka_unit_test(canonical_l3_ipv4_hash_matches_ethernet_hash),
		cmocka_unit_test(canonical_l3_ipv6_hash_matches_ethernet_hash),
		cmocka_unit_test(canonical_l3_hash_imports_hardware_rss),
		cmocka_unit_test(ipv4_fragments_share_one_hash),
		cmocka_unit_test(canonical_ipv4_fragments_share_one_hash),
		cmocka_unit_test(canonical_validity_is_cleared_on_reset),
		cmocka_unit_test(decap_reseed_quarantines_outer_rss),
		cmocka_unit_test(decap_without_rss_invalidates_outer_canonical_hash),
	};
